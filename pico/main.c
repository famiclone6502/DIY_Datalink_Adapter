...
int main()  {
    ...
    const uint led_pin = 25; // Use built in led on Pico, or change to 18 for external LED (with 1k resistor between GND)
    gpio_init(led_pin); // Initialize LED pin
    gpio_set_dir(led_pin, GPIO_OUT); // Initialize LED pin
